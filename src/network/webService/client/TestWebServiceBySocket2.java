...
public class TestWebServiceBySocket2 {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("original http request content after analysis:\r\n"
					+ SocketUtils2.doSocket(WebServiceServer.webServiceUrl, message, Constant.defaultCharset));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private static String getHttpPostHeader(URL url, int contentLength) {
		StringBuffer header = new StringBuffer().append("POST ").append(url.getPath()).append(" HTTP/1.0\r\n")
